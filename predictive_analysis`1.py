...
def data_prepare(fn, sel=["PhaseTime","rdFar"], goal="Linebreak", verbosity=0):
    ...
    all_columns = ['PhaseID', 'PhaseTime', 'PhaseName', 'RucksNo', 'PosGainline', 'NegGainline', 'Linebreak', 'LineBreakPrevPhase', 'rdNear', 'rdRuck', 'rdFar', 'rdMiddle', 'rsSlow', 'rsMedium', 'rsFast', 'KickTotal', 'KickedInfield', 'KickedtoTouch', 'turnoverTot', 'turnoverKick', 'turnoverContact', 'fromTurnover', 'penFK', 'penContact', 'Tries', 'TryTotal', 'PenTotal']

    all_names = ['PhaseTime', 'RucksNo', 'rdNear', 'rdRuck', 'rdFar', 'rdMiddle', 'KickTotal', 'KickedInfield', 'turnoverTot', 'turnoverKick', 'turnoverContact', 'fromTurnover', 'penFK', 'penContact']
    two_names = ["PhaseTime","rdFar"]

    ep1 = ['Kick_at_Goal_Success', 'Drop_Goals',
       ...
       'Yellow_Cards', 'Red_Cards', 'Tot_Own_Scrums', 'Penalties_Awarded']
    epall = ep1 + ['Tries_Scored',]
    ...
    if isinstance(sel,(list,tuple)) or sel=='two':
        ...
        X = np.array(data_df[ep1].values)
        X = preprocessing.scale(X,axis=0)
        names = ep1
    elif sel=='epall':
        X = np.array(data_df[epall].values)
        X = preprocessing.scale(X,axis=0)
        names = epall
    else: 
        if sel == 'alll':
            all_names = all_names+['Linebreak']
...
def game_analysis(fn,sclf,ax=None,sel=["PhaseTime","rdFar"],toest=False,verbosity=0):
    X, y, names = data_prepare(fn, sel=sel, goal="Linebreak", verbosity=verbosity-1)
...
def predict_evaluate_models(fn ,ax=None, sel=["PhaseTime","rdFar"], goal="Linebreak", verbosity=0):
    class_weight = 'auto'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-fn', type=str, default='7bips.csv',help="filename to apply cmd")
