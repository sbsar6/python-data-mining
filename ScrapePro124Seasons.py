...
def https():
        ...
        for i in range (233203,233472,2):
            temp = ("http://en.espn.co.uk/guinness-pro12-2014-15/rugby/match/" + str(i)+".html?view=scorecard")
            urlList.append(temp)
        for i in range (191311,191580,2):
            temp = ("http://en.espn.co.uk/rabodirect-pro12-2013-14/rugby/match/" + str(i)+".html?view=scorecard")
            urlList.append(temp)
        for i in range (166984,167119,1):
            temp = ("http://en.espn.co.uk/rabodirect-pro12-2012-13/rugby/match/" + str(i)+".html?view=scorecard")
            urlList.append(temp)
        for i in range (144587,144722,1):
            temp = ("http://en.espn.co.uk/rabodirect-pro12-2011-12/rugby/match/" + str(i)+".html?view=scorecard")
            urlList.append(temp)
...
Data = pd.DataFrame(columns = ['Team_Name',
                                   ...
                                   'Opp Scrum Won',
                                   'Opp Scrum Lost',
                                   'Percent Scrums on own feed',
                                   'Own Lineout Won', 
                                   'Own Lineout Lost',
                                   'Opp Lineout Won',
                                   'Opp Lineout Lost',
                                   'Percent Lineouts on own feed',
                                   'Penalties Conceeded',
                                   'Yellow Cards',
                                   'Red Cards',
                                   'Referee',
    ...
    for i in range (0,3,2):
        ...
        if len(lists) == 29:
            Data= Data.append({'Team_Name':lists[0][i],
                           ...
                           'Percent Rucks Won':lists[14][i],
                           'Mauls Won': maulsWon,
                           'Total Mauls': totMauls,
                           'Percent Mauls Won':lists[15][i],
                ...
                Data= Data.append({'Team_Name':lists[0][i],
                           ...
                           'Rucks Won': rucksWon,
                           'Total Rucks': totRucks,
                           'Percent Rucks Won':lists[13][i],
                           'Mauls Won': maulsWon,
                           'Total Mauls': totMauls,
                           'Percent Mauls Won':lists[14][i],
                           ...
                           'Opp Scrum Won':oppScrumWon,
                           'Opp Scrum Lost':oppScrumLost,
                           'Percent Scrums on own feed':lists[19][i],
                           'Own Lineout Won':ownLineWon, 
                           'Own Lineout Lost':ownLineLost,
                           'Opp Lineout Won':oppLineWon,
                           'Opp Lineout Lost':oppLineLost,
                           'Percent Lineouts on own feed':lists[20][i],
...
Data['Percent_Mauls_Won'] = Data['Percent_Mauls_Won'].apply(lambda x: str(x).replace("0 from 0","0")[0])
Data['Percent_Scrums_on_own_feed'] = Data['Percent_Scrums_on_own_feed'].apply(lambda x: str(x).replace("0 won, 0 lost","0")[0])
Data['Percent_Lineouts_on_own_feed'] = Data['Percent_Lineouts_on_own_feed'].apply(lambda x: str(x).replace("0 won, 0 lost","0")[0])
...
Data.to_csv("Pro12FourSeasons.csv")
