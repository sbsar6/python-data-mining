...
def game_analysis(fn,sclf,ax=None,sel=["Penalties_Conceeded","Tries_Scored"],toest=False,verbosity=0):
    X, y, names = data_prepare(fn, sel=sel, goal="Referee", verbosity=verbosity-1)
...
def feature_selection_ET(fn ,ax=None, sel="all", goal="Referee", verbosity=0, nf=7):
    n_estimators=500
    X, y, names = data_prepare(fn, sel=sel, goal=goal, verbosity=verbosity-1)
...
def test(args):
    ...
    if False:    
        fig,ax = plt.subplots()
        feature_selection_ET(args.fn ,ax=ax, sel="all", goal="Referee", verbosity=2)
        plt.show()
    if True:
        fig,ax = plt.subplots()
        feature_selection_RFE(args.fn ,ax=ax, sel="all", goal="Referee", verbosity=2)
        plt.show()
